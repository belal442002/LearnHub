...
{
    public class CustomCheckUpdatedAnswersAttribute : ValidationAttribute
    {
        public CustomCheckUpdatedAnswersAttribute()
        {
            var defaultMessage = "Wrong updating Answers format";
            ErrorMessage ??= defaultMessage;
        }

        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            var answers = value as List<UpdateQBAnswersRequestDto>;
            if (answers == null)
            {
